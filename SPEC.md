...
6. **Every hop between sources must be explicit.** A citation chain is a sequence of discrete hops; each hop must record which document cited it and full source metadata before any quotation appears. For every hop after the first, record whether the *previous* hop's document **explicitly cites** this source (`explicitly_cited_by_previous`): `true` when the link is a real citation you can point to in the previous document, `false` when the connection was established some other way (identified independently, editorial knowledge, matched by content). When it is `true`, also copy the reference **exactly as printed in the previous document** into `citation_in_previous_verbatim` — the bibliography line or footnote, verbatim, not paraphrased (this is a factual reference string, not copyrightable prose). When it is `false` or you could not locate the reference, leave `citation_in_previous_verbatim` null.
...
```
{
  ...
  "explicitly_cited_by_previous": boolean | null,  // hop_index > 1 only: does the PREVIOUS hop's document explicitly cite this source? null on hop 1, or when not determined
  "citation_in_previous_verbatim": string | null,  // if explicitly cited, the reference/citation text exactly as it appears in the previous hop's document (bibliography entry or footnote); null otherwise
  "source": {
