...
// link. Search by ISBN when we have one, else by title words + author surname
// (restricted to `mediatype:texts`, and only when we have BOTH so a fuzzy
// title match can't quietly pull the wrong book). Returns direct-download URLs
// for the full-text PDF and the plain-text OCR, PDF first.
async function archiveOrgCandidates(source) {
  ...
  let q = null;
  if (isbn.length === 10 || isbn.length === 13) q = "isbn:" + isbn;
  else if (titleWords && surname) q = "mediatype:texts AND title:(" + titleWords + ") AND creator:(" + surname + ")";
  if (!q) return out;

  const j = await tryJSON(
    "https://archive.org/advancedsearch.php?q=" + encodeURIComponent(q) + "&fl[]=identifier&rows=5&output=json&sort[]=downloads+desc"
  );
...
function reasonRank(reason) {
  ...
  if (/anti-bot|access denied|lending-restricted/i.test(reason)) return 3;
