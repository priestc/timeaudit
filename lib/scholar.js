...
async function tryJSON(url, retries = 0) {
  for (let i = 0; i <= retries; i++) {
    try {
      const r = await politeGet(url);
      if (r.status === 200) return JSON.parse(r.body);
    } catch {
      /* optional source — a slow / flaky host (archive.org especially) */
    }
...
// link. Tries an ISBN search first, then a distinctive-title + author-surname
// search (an ISBN from a modern reprint often isn't on the old PD scan). Every
// hit is checked against the citation's own title and author before it is
// trusted, because IA's fuzzy matching will otherwise return a wrong book for
// a generic title. Returns direct-download URLs for the full-text PDF and the
// plain-text OCR, PDF first.
async function archiveOrgCandidates(source, diag) {
  const out = [];
  const note = (result, detail) => {
    if (diag) diag.push({ via: "internet archive", url: "https://archive.org/advancedsearch.php", result, detail });
  };
  ...
  const surnameRe = surname ? new RegExp("\\b" + surname.replace(/[^A-Za-zÀ-ɏ]/g, "") + "\\b", "i") : null;

  const queries = [];
  if (isbn.length === 10 || isbn.length === 13) queries.push({ q: "mediatype:texts AND isbn:" + isbn, checkTitle: false });
  // a fuzzy title search needs ≥2 distinctive words AND an author surname
  if (wantTok.length >= 2 && surname) {
    queries.push({ q: "mediatype:texts AND title:(" + wantTok.slice(0, 8).join(" ") + ") AND creator:(" + surname + ")", checkTitle: true });
  }
  if (!queries.length) return out;

  let searchFailed = false;
  let sawCandidates = false;
  for (const { q, checkTitle } of queries) {
    const j = await tryJSON(
      "https://archive.org/advancedsearch.php?q=" +
        encodeURIComponent(q) +
        "&fl[]=identifier&fl[]=title&fl[]=creator&fl[]=mediatype&rows=8&output=json&sort[]=downloads+desc",
      1
    );
    if (!j) searchFailed = true;
    const docs = ((j && j.response && j.response.docs) || [])
      .filter((d) => d.identifier && d.mediatype === "texts" && !/^bwb[_-]|_daily_pallets/.test(d.identifier))
      // best title overlap first, so "…: Rome" beats a bare "…" or a "Vol. I"
      .map((d) => ({ d, ov: wantTok.filter((w) => titleTokens(d.title).includes(w)).length }))
      .sort((a, b) => b.ov - a.ov)
      .map((x) => x.d);
    for (const d of docs.slice(0, 6)) {
      ...
      const titleOk = !checkTitle
        ? overlap >= 1 || !wantTok.length // ISBN match: a light title touch is enough
        : overlap >= 2 && overlap >= Math.ceil(wantTok.length * 0.5);
      const creatorOk = !surnameRe || surnameRe.test(String(d.creator || ""));
      if (!titleOk || !creatorOk) continue;
      const meta = await tryJSON("https://archive.org/metadata/" + encodeURIComponent(d.identifier), 1);
      ...
      if (pdf || txt) return out; // first usable item across all queries wins
    }
  }
  // nothing usable — say why, so the claim's retrieval_history shows the attempt
  if (searchFailed) {
    note("network_error", "the Internet Archive search did not respond (it is often slow or rate-limited) — re-running the analysis may pick it up");
  } else if (sawCandidates) {
    note("no_match", "the Internet Archive has scans of this title but none matched the citation's title + author closely enough to trust");
  } else {
    note("no_match", "no Internet Archive scan found for this title + author");
  }
  return out;
}

// When a citation has no URL and no DOI (a bare {{sfn}} to a book chapter, a
// print-only reference), look the work up in the scholarly metadata services
// — the scriptable equivalent of "search for it and read the results" — to
// recover a title / author / year / container / identifier and, ideally, a DOI
// or landing page that retrieval can then use. Every lookup is logged to
// `metadata_history`; a confident match is merged into the source (nulls only).
function nameHits(surname, names) {
  if (!surname) return true;
  const re = new RegExp("\\b" + surname.replace(/[^A-Za-zÀ-ɏ]/g, "") + "\\b", "i");
  return names.some((x) => re.test(String(x || "")));
}
async function enrichMetadata(source, { email, budget } = {}) {
  const history = [];
  if (source.retrieval_url || source._doi) return history;
  // one shared per-run cap so a big page doesn't fire hundreds of API calls
  if (budget) {
    if (budget.enrich == null) budget.enrich = 30;
    if (budget.enrich <= 0) {
      history.push({ via: "metadata search", query: null, result: "skipped", detail: "metadata-lookup budget for this run was exhausted" });
      return history;
    }
  }

  const author = Array.isArray(source.author) ? source.author[0] : source.author;
  const surname = String(author || "").split(/[\s,]+/).filter(Boolean).pop();
  const wantTok = titleTokens(source.title);
  const seenPart = new Set();
  const q = [author, source.year, source.title, source.container_work, source.publisher_or_journal]
    .filter(Boolean)
    .map((x) => String(x).trim())
    .filter((x) => {
  ...
  if (wantTok.length < 3 && !(surname && source.year)) {
    history.push({
      via: "metadata search",
      query: q || null,
      result: "skipped",
      detail: "too little to search on — need a multi-word title, or an author surname plus a year",
    });
    return history;
  }
  if (budget) budget.enrich--;

  const merge = (found) => {
    if (found.title && (!source.title || source.title.length < 6)) source.title = found.title;
    if (found.author && found.author.length && !source.author) source.author = found.author.length === 1 ? found.author[0] : found.author;
    if (found.year && !source.year) source.year = found.year;
    // a recovered pre-1930 year makes it public-domain — which changes how
    // retrieval treats a Google Books / Internet Archive result
    if (found.year && found.year < 1930 && source.is_public_domain == null) source.is_public_domain = true;
    if (found.container && !source.container_work) source.container_work = found.container;
    if (found.container && !source.publisher_or_journal) source.publisher_or_journal = found.container;
    if (found.doi) {
      source._doi = found.doi;
      source.identifier = source.identifier || "doi:" + found.doi;
      source.retrieval_url = source.retrieval_url || "https://doi.org/" + found.doi;
    }
    if (found.isbn && !source._isbn) {
      source._isbn = found.isbn;
      source.identifier = source.identifier || "ISBN " + found.isbn;
    }
    if (found.gbId) {
      source._gbId = source._gbId || found.gbId;
      source.retrieval_url = source.retrieval_url || "https://books.google.com/books?id=" + found.gbId;
    }
  };

  // --- Google Books first: its catalogue coverage of books is the best, and it
  //     hands back title / author / year / publisher / ISBN + a volume id that
  //     then drives the ISBN'd Internet Archive search and the Google Books
  //     download attempt. (Keyless quota is small — a 429 is logged, not fatal;
  //     set GOOGLE_BOOKS_API_KEY in .env to raise it.) ---
  try {
    const qParts = [];
    if (source.title)
      qParts.push(
        'intitle:"' + String(source.title).replace(/["]/g, "").replace(/[:;].*$/, "").replace(/\s+/g, " ").trim().split(/\s+/).slice(0, 10).join(" ") + '"'
      );
    if (surname) qParts.push("inauthor:" + surname);
    const gq = qParts.join(" ") || q;
    const key = process.env.GOOGLE_BOOKS_API_KEY;
    const j = await tryJSON(
      "https://www.googleapis.com/books/v1/volumes?maxResults=5&country=US&q=" + encodeURIComponent(gq) + (key ? "&key=" + encodeURIComponent(key) : "")
    );
    if (!j) {
      history.push({
        via: "google books",
        ...
        found: null,
        detail: key
          ? "Google Books API unreachable"
          : "Google Books API returned no data — the keyless daily quota is likely exhausted; set GOOGLE_BOOKS_API_KEY in .env",
      });
    } else {
      const its = j.items || [];
      let gb = null;
      ...
      for (const it of its) {
        ...
        const got = titleTokens((v.title || "") + " " + (v.subtitle || ""));
        const overlap = wantTok.filter((w) => got.includes(w)).length;
        const frac = wantTok.length ? overlap / wantTok.length : 0;
        const authOk = nameHits(surname, v.authors || []);
        const yr = (String(v.publishedDate || "").match(/\d{4}/) || [])[0];
        const yearOk = source.year && yr ? Math.abs(+yr - +source.year) <= 1 : false;
        let score = 0;
        if (wantTok.length >= 3 && overlap >= 3 && frac >= 0.5) score += 2;
        if (authOk) score += 1;
        if (yearOk) score += 1;
        if (score > gbScore) {
          gbScore = score;
      ...
      if (gb && gbScore >= 3) {
        ...
        const yr = (String(v.publishedDate || "").match(/\d{4}/) || [])[0];
        const found = {
          title: (v.title || null) && v.title + (v.subtitle ? ": " + v.subtitle : ""),
          author: v.authors || [],
          year: yr ? +yr : null,
          container: v.publisher || null,
          doi: null,
          isbn: isbn ? isbn.replace(/[- ]/g, "") : null,
          gbId: gb.id,
        };
        merge(found);
        history.push({ via: "google books", query: gq, result: "enriched", found, detail: "match score " + gbScore + "/4 (volume " + gb.id + ")" });
        return history;
      }
      history.push({
        via: "google books",
        query: gq,
        result: its.length ? "low_confidence" : "no_match",
        found: null,
        detail: its.length ? "best of " + its.length + " Google Books results scored only " + gbScore + "/4" : "no results",
      });
    }
  } catch (e) {
    history.push({ via: "google books", query: null, result: "network_error", found: null, detail: String((e && e.message) || e) });
  }

  // --- Crossref: built for "here is a messy citation, find the work" ---
  try {
    const j = await tryJSON(
      "https://api.crossref.org/works?rows=3&select=DOI,title,container-title,publisher,issued,author,type,URL&query.bibliographic=" +
        encodeURIComponent(q) +
        (email ? "&mailto=" + encodeURIComponent(email) : "")
    );
    let items = (j && j.message && j.message.items) || [];
    // A bare {{sfn}} with no DOI is a book or book chapter — a journal article
    // would carry its own DOI. Crossref's record for a book *review* is a
    // journal-article whose title echoes the reviewed book (title + author +
    // publisher), which scores a fake match. So for a book-shaped source, only
    // trust Crossref hits that are themselves book-shaped.
    const bookishSrc = !source.document_type || /book|chapter|monograph|thesis|other/i.test(source.document_type);
    const BOOK_TYPE = /^(book|monograph|reference-book|edited-book|book-set|book-part|book-chapter|book-section|book-track|dissertation)$/;
    if (bookishSrc) items = items.filter((it) => BOOK_TYPE.test(it.type || ""));
    let best = null;
    let bestScore = 0;
    for (const it of items) {
      const itTitle = (it.title || [])[0] || "";
      if (/^review of\b|\breviewed by\b/i.test(itTitle)) continue;
      const gotTok = titleTokens(itTitle);
      const overlap = wantTok.filter((w) => gotTok.includes(w)).length;
      const frac = wantTok.length ? overlap / wantTok.length : 0;
      const authOk = nameHits(surname, (it.author || []).map((a) => (a.family || "") + " " + (a.given || "") + " " + (a.name || "")));
      const yr = it.issued && it.issued["date-parts"] && it.issued["date-parts"][0] && it.issued["date-parts"][0][0];
      const yearOk = source.year && yr ? Math.abs(+yr - +source.year) <= 1 : false;
      let score = 0;
      // a real title match needs our parsed title to itself be substantial
      // (>= 3 distinctive words) and to overlap the candidate's title by >= 3 —
      // 2 shared words is far too loose and lets a review or a namesake through
      if (wantTok.length >= 3 && overlap >= 3 && frac >= 0.5) score += 2;
      if (authOk) score += 1;
      if (yearOk) score += 1;
      if (score > bestScore) {
    ...
    if (best && bestScore >= 3) {
      const yr = best.issued && best.issued["date-parts"] && best.issued["date-parts"][0] && best.issued["date-parts"][0][0];
      const found = {
        title: (best.title || [])[0] || null,
        author: (best.author || []).map((a) => [a.given, a.family].filter(Boolean).join(" ") || a.name).filter(Boolean),
        year: yr ? +yr : null,
        container: (best["container-title"] || [])[0] || best.publisher || null,
        doi: best.DOI || null,
        isbn: null,
      };
      merge(found);
      history.push({ via: "crossref", query: q, result: "enriched", found, detail: "match score " + bestScore + "/4" });
      return history;
    }
    history.push({
      via: "crossref",
      query: q,
      result: items.length ? "low_confidence" : "no_match",
      found: null,
      detail: items.length ? "best of " + items.length + " results scored only " + bestScore + "/4 — not adopted" : "no results",
    });
  } catch (e) {
    history.push({ via: "crossref", query: q, result: "network_error", found: null, detail: String((e && e.message) || e) });
  }

  // --- Open Library: for books, to recover an ISBN — only when Crossref drew
  //     a total blank (a low-confidence Crossref hit means it isn't there) ---
  const bookish = !source.document_type || /book|other|chapter|thesis/i.test(source.document_type);
  const crossrefBlank = history.length && history[history.length - 1].result === "no_match";
  if (!source._doi && !source._isbn && bookish && crossrefBlank) {
    try {
      const j = await tryJSON(
        "https://openlibrary.org/search.json?limit=3&fields=title,author_name,first_publish_year,isbn,key&q=" +
          encodeURIComponent([source.title, author].filter(Boolean).join(" "))
      );
      const docs = (j && j.docs) || [];
      let best = null;
      let bestScore = 0;
      for (const it of docs) {
        const gotTok = titleTokens(it.title);
        const overlap = wantTok.filter((w) => gotTok.includes(w)).length;
        const frac = wantTok.length ? overlap / wantTok.length : 0;
        const authOk = nameHits(surname, it.author_name || []);
        let score = 0;
        if (wantTok.length >= 3 && overlap >= 3 && frac >= 0.5) score += 2;
        if (authOk) score += 1;
        if (source.year && it.first_publish_year && Math.abs(+it.first_publish_year - +source.year) <= 2) score += 1;
        if (score > bestScore) {
          bestScore = score;
          best = it;
        }
      ...
      if (best && bestScore >= 3) {
        const isbn = (best.isbn || []).find((x) => /^(97[89])?\d{9}[\dxX]$/.test(String(x).replace(/[- ]/g, ""))) || (best.isbn || [])[0];
        const found = {
          ...
          container: null,
          doi: null,
          isbn: isbn ? String(isbn).replace(/[- ]/g, "") : null,
        };
        merge(found);
        history.push({ via: "open library", query: [source.title, author].filter(Boolean).join(" "), result: "enriched", found, detail: "match score " + bestScore + "/4" });
      } else {
        history.push({
          via: "open library",
          query: [source.title, author].filter(Boolean).join(" "),
          result: docs.length ? "low_confidence" : "no_match",
          found: null,
          detail: docs.length ? "best of " + docs.length + " scored only " + bestScore + "/4" : "no results",
        });
      }
    } catch (e) {
      history.push({ via: "open library", query: null, result: "network_error", found: null, detail: String((e && e.message) || e) });
    }
  }
  return history;
}
...
async function resolveCandidates(source, { email, diag } = {}) {
  const out = [];
  ...
  if (looksLikeBook) {
    for (const c of await archiveOrgCandidates(source, diag)) push(c.url, c.why);
  }
  ...
  const gbId = source._gbId || ((source.retrieval_url || "").match(/[?&]id=([\w-]+)/) || [])[1];
  if (gbId && (source.is_public_domain === true || /book|other|chapter/i.test(source.document_type || ""))) {
    push("https://books.google.com/books/download/book.pdf?id=" + gbId + "&output=pdf", "google books pdf");
  }
...
// A short, stable tag for one retrieval attempt's outcome — what goes in each
// retrieval_history[] step's `result`. Free-text `detail` carries the specifics.
function resultCode(reason) {
  const r = String(reason || "");
  if (/^ok$/i.test(r) || /\bretrieved\b/i.test(r)) return "retrieved";
  if (/copyrighted/i.test(r)) return "copyright";
  if (/public-domain book, but no downloadable/i.test(r)) return "no_free_scan";
  if (/lending-restricted/i.test(r)) return "lending_restricted";
  if (/CAPTCHA/i.test(r)) return "captcha";
  if (/login \/ subscription|access-restricted/i.test(r)) return "login_wall";
  if (/paywalled|subscription|institutional access|requires purchase/i.test(r)) return "paywall";
  if (/anti-?bot|browser-verification/i.test(r)) return "bot_wall";
  if (/access denied|server refused access/i.test(r)) return "access_denied";
  const http = r.match(/HTTP (\d{3})/);
  if (http) return "http_" + http[1];
  if (/exceeded the .* size cap/i.test(r)) return "too_large";
  if (/was not actually a PDF|not actually a PDF/i.test(r)) return "wrong_type";
  if (/almost no text|stub or preview/i.test(r)) return "stub_page";
  if (/redirect chain .* never resolved/i.test(r)) return "no_document";
  if (/network error|timeout/i.test(r)) return "network_error";
  if (/budget/i.test(r)) return "skipped_budget";
  if (/preview only, not the full text/i.test(r)) return "copyright";
  if (/empty response/i.test(r)) return "empty";
  if (/no retrievable URL/i.test(r)) return "no_url";
  return "failed";
}
const trimUrl = (u) => (u ? String(u).slice(0, 300) : null);
...
async function fetchSource(source, { cacheDir, pageSlug, email, budget }) {
  ...
  for (const ext of ["pdf", "html", "xml", "txt"]) {
    ...
    if (fs.existsSync(p)) {
      ...
      return {
        ...
        metadataHistory: Array.isArray(meta.metadataHistory) ? meta.metadataHistory : [],
      };
    }
  }
  ...
  // fill in missing title / author / DOI / URL by looking the work up in the
  // scholarly metadata services before deciding where to fetch it from
  const metadataHistory = await enrichMetadata(source, { email, budget });

  // ordered log of every place retrieval looked and what came back
  //   { via, url, result, detail }
  const history = [];
  const step = (via, url, reason, ok) =>
    history.push({
      via,
      url: trimUrl(url),
      result: ok ? "retrieved" : resultCode(reason),
      detail: ok ? null : reason || null,
    });

  // `diag` collects notes from searches that ran but yielded no candidate
  // (e.g. an Internet Archive title search that timed out or matched nothing)
  const diag = [];
  const cands = await resolveCandidates(source, { email, diag });
  for (const d of diag) history.push({ via: d.via, url: trimUrl(d.url), result: d.result, detail: d.detail });

  if (!cands.length) {
    const reason =
      "no retrievable URL could be resolved from this citation's metadata (e.g. a print-only reference with no DOI or online copy)";
    step("citation metadata", source.retrieval_url || null, reason, false);
    return { status: "unreachable", reason, history, metadataHistory };
  }

  const failures = []; // { reason } for every candidate that didn't work
  for (const c of cands) {
    if (budget && budget.left <= 0) {
      step(c.why, c.url, "download budget for this run was exhausted before this candidate", false);
    ...
    if (result.ok) step(c.why, result.url, null, true);
    else step(c.why, c.url, result.reason, false);
    ...
    if (!result.ok && ARCHIVABLE_WHY.has(c.why) && budget && budget.left > 0) {
      ...
      if (snapUrl) {
        ...
        budget.left--;
        step("wayback machine", snapUrl, snapResult.ok ? null : snapResult.reason, snapResult.ok);
        if (snapResult.ok) {
          result = snapResult;
    ...
    if (result.ok) {
      ...
      writeMeta(result.file, { via, viaWayback: !!result.viaWayback, history, metadataHistory });
      return {
        status: "downloaded",
        file: result.file,
  ...
  return { status: "unreachable", reason: (best && best.reason) || "every candidate URL failed", history, metadataHistory };
}
