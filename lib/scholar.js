...
async function fetchSource(source, { cacheDir, pageSlug, email, budget }) {
  ...
  if (!cands.length) {
    ...
    return { status: "unreachable", reason, history };
  }
  ...
  for (const c of cands) {
    ...
    if (result.ok) {
      ...
      writeMeta(result.file, { via, viaWayback: !!result.viaWayback, history });
      return {
        status: "downloaded",
        file: result.file,
  ...
  return { status: "unreachable", reason: (best && best.reason) || "every candidate URL failed", history };
}
