...
async function fetchSource(source, { cacheDir, pageSlug, email, budget }) {
  ...
  for (const c of cands) {
    ...
    try {
      ...
      failures.push({ reason: "network error contacting " + c.url + " (" + (e && e.message ? e.message : e) + ")" });
      continue;
    }
    if (budget) budget.left--;
    if (r.status !== 200) {
      failures.push({ reason: "HTTP " + r.status + " from " + c.url });
      continue;
    }
    if (!r.body.length) {
      failures.push({ reason: "empty response from " + c.url });
      continue;
    }
    const ext = extRfrom(r.contentType, r.finalUrl);
    if (ext === "pdf" && r.body.slice(0, 5).toString("latin1") !== "%PDF-") {
      failures.push({ reason: "response from " + c.url + " was not actually a PDF" });
      continue;
    }
    if (ext !== "pdf") {
      ...
      if (blocked) {
        failures.push({ reason: blocked });
        continue; // bot-wall / paywall — try the next candidate
      }
      if (asText.replace(/<[^>]+>/g, "").replace(/\s+/g, " ").trim().length < 400) {
        failures.push({ reason: "page returned almost no text (likely a stub or preview page)" });
        continue; // stub page
      }
    }
