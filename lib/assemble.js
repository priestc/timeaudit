...
function buildHop(hop, i) {
  ...
  const quotes = (hop.verbatim_quotes || []).slice(0, limit);
  return {
    ...
    // screenshot of each quote as printed in the source PDF, aligned by index
    // to verbatim_quotes; null where the quote could not be located
    quote_images: quotes.map((_, k) => (hop.quote_images && hop.quote_images[k]) || null),
    // the full page each quote was cropped from (authenticity), same alignment
    quote_page_images: quotes.map((_, k) => (hop.quote_page_images && hop.quote_page_images[k]) || null),
...
function buildClaim(prefix, n, claim) {
  ...
  const out = {
    ...
    // screenshot of this sentence on the rendered Wikipedia page + the full page
    wikipedia_quote_image: claim.wikipedia_quote_image || null,
    wikipedia_quote_page_image: claim.wikipedia_quote_page_image || null,
