...
function buildHop(hop, i) {
  ...
  return {
    ...
    // Every hop after the first is, by SPEC Rule 6, a source the previous
    // document explicitly cites; this is that citation's text, verbatim.
    citation_in_previous_verbatim: i === 0 ? null : hop.citation_in_previous_verbatim || null,
    // set only on a hop that starts its own branch, when the sentence cites
    // more than one source in parallel (e.g. "[98][99]" on two works) — this
    // one is independent, not a continuation of the hop before it
    ...
    verbatim_quotes: (hop.verbatim_quotes || []).slice(0, limit),
