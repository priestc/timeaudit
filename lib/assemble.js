...
function cleanSource(s) {
  ...
  const out = {
    ...
    // the "Retrieved <date>" an editor recorded on the Wikipedia citation —
    // when the online source was last verified live at retrieval_url
    wikipedia_access_date: s._accessDate ?? s.wikipedia_access_date ?? null,
    // phase 1-2: "retrieved" (file is in the cache) | "unreachable" (download
    // failed). "verified_verbatim" — the source's text actually checked against
    // the claim — is a phase-3 outcome and not emitted yet.
    retrieval_status: s.retrieval_status || "retrieved",
    // free-text reason retrieval didn't reach "verified_verbatim" — set
    // whenever the pipeline knows something more specific than the bare
    // status, e.g. "unable to retrieve source because copyrighted"
    retrieval_note: s.retrieval_note ?? null,
    // true when the file at local_cache_path came from a Wayback Machine
    // archived snapshot because the live URL failed, not from the live URL
    retrieved_via_wayback: s.retrieved_via_wayback === true,
    // ordered log of every retrieval attempt for this source: each step is
    // { via, url, result, detail } — `via` names where it looked (cited url,
    // openalex OA pdf, ncbi bioc, internet archive pdf, wayback machine, …),
    // `result` a short tag (retrieved | http_404 | copyright | paywall |
    // login_wall | lending_restricted | captcha | no_document | …). New
    // retrieval backends just add their own `via` label here.
    retrieval_history: Array.isArray(s.retrieval_history)
      ? s.retrieval_history.map((e) => ({
          via: e.via ?? null,
          url: e.url ?? null,
      ...
      : [],
    // ordered log of the scholarly-metadata lookups used to flesh out a
    // citation that arrived with no URL / DOI: each step is
    // { via, query, result, found, detail } — `via` is the service
    // (crossref, open library, …), `found` the fields it returned on a match.
    metadata_history: Array.isArray(s.metadata_history)
      ? s.metadata_history.map((e) => ({
          via: e.via ?? null,
          query: e.query ?? null,
          result: e.result ?? null,
          found: e.found ?? null,
...
function buildHop(hop) {
  const limit = hop.source && hop.source.is_public_domain ? 8 : 3;
  return {
    hop_index: hop.hop_index,
    cited_by: hop.cited_by,
    // set only when the citing sentence cites more than one source in parallel
    // (e.g. "[98][99]" on two works) — each is an independent Wikipedia
    // citation, fetched on its own. null on a single-source sentence.
    parallel_citation: hop.parallel ? { index: hop.parallel.index, total: hop.parallel.total } : null,
    source: cleanSource(hop.source),
    // passage(s) a Wikipedia explanatory ("[m]"/"[n]") note on the citing
    // sentence quotes from this source — editorially selected by Wikipedia,
    // not text-mined here; same Rule 4 quote cap applies
    wikipedia_note_quotes: (hop.wikipedia_note_quotes || []).slice(0, limit),
    // NOTE: is_terminal / terminal_type / structured_facts / verbatim_quotes /
    // citation_in_previous_verbatim are phase-3 outputs (classifying the
    // downloaded source text). Phase 3 is not implemented yet, so they are
    // omitted from the report rather than written as empty placeholders.
  };
}

function buildClaim(prefix, n, claim) {
  const id = prefix + "-" + String(n).padStart(3, "0");
  // "Citations on the sentence" = real numbered citations only. A lettered
  // {{efn}} note ([m]/[n]) is not a citation — its snippet is attached to the
  // source it quotes, as hop.wikipedia_note_quotes.
  const citing = (claim.markers || []).filter((mk) => !mk.isNote);
  const footnotes = {};
  for (const mk of citing) {
  ...
  const out = {
    claim_id: id,
    // verbatim including the [n] citation markers exactly where Wikipedia shows them
    wikipedia_text_verbatim: claim.sentence_cited || claim.sentence,
    location_on_page: claim.section,
    citation_markers: [...new Set(citing.map((m) => m.label))],
    citation_footnotes_verbatim: footnotes,
    citation_chain: chain,
    // phase 1-2: "retrieved" | "dead_end" | "no_source". "resolved" / "pending"
    // are phase-3 outcomes (see SPEC "Implementation status").
    status: claim.status || "no_source",
