  ...
  const hops = claim.hops
    .map((h, i) => {
      ...
      return (
        ...
        (i + 1) +
        (h.parallel
          ? " (parallel citation " + h.parallel.index + " of " + h.parallel.total + " — an independent source Wikipedia cites alongside the other(s), NOT a continuation of the previous hop)"
          : "") +
        "\n  source: " +
        JSON.stringify({
          author: s.author,
