...
// MediaWiki's current parser output entity-encodes the characters that would be
// unsafe in an id="" / href="" fragment — underscore especially — so a full
// reference list item comes back as <li id="cite&#95;note-19">, not
// <li id="cite_note-19">. Every reference/marker regex in this file matches the
// literal "cite_note-…", so fold those few structural entities back before
// anything parses the HTML. (The untouched API response is kept in
// <slug>.parse.json.) href="" fragments are already emitted literally, so after
// this the two forms agree and markers join to their reference text again.
//
// &#39; matters too: a {{sfn}} to a classical source ("Flor. ''Epit.'' i.11–12")
// puts the italic-markup apostrophes into the CITEREF anchor, so the note id is
// `…Flor._&#39;&#39;Epit.&#39;&#39;i.11–12-11`. The marker side is decoded to `'`
// downstream (decodeEntities) but the <li id> is not, so they miss and the
// footnote text is lost — fold &#39; here so both sides read `''`.
const STRUCT_ENTITY = { "&#95;": "_", "&#91;": "[", "&#93;": "]", "&#58;": ":", "&#39;": "'" };
function normalizeParserHtml(html) {
  return String(html).replace(/&#(?:95|91|93|58|39);/g, (m) => STRUCT_ENTITY[m]);
}
...
async function fetchPage(input, opts = {}) {
  ...
  const html = normalizeParserHtml(p.text);
  const out = {
    title: p.title,
    slug,
    lang,
  ...
  if (opts.cacheDir) {
    ...
    // cache the normalized HTML — lib/context.js re-parses this file and must
    // see the same thing the extractor did
...
function buildReferenceIndex(html) {
  ...
  // 1a. every <cite|span id="CITEREF..."> -> its COinS span (if any) and the
  // plain text of the bibliography entry. Many hand-built bibliographies use
  // <span id="CITEREF..." class="citation"> with NO COinS blob, so the text is
  // the only machine-readable form of "Lintott, A. (1994a). "The Roman empire
  // and its problems…". In CAH² 9 (1994), pp. 16–39."
  const citerefCoins = {}; // CITEREFxxx -> parsed source
  const citerefArchive = {}; // CITEREFxxx -> { archiveUrl, accessDate }
  const citerefText = {}; // CITEREFxxx -> plain-text bibliography entry
  const citerefTitle = {}; // CITEREFxxx -> the italicised title in the entry, if any
  let m;
  const citeIdRe = /<(cite|span)[^>]*\bid="(CITEREF[^"]+)"[^>]*>/g;
  while ((m = citeIdRe.exec(h))) {
    const tag = m[1];
    // `entry` is this element's own content, up to its matching close tag —
    // used for the bibliography text / title / archive link so a neighbour's
    // can't bleed in.
    const entry = h.slice(m.index, m.index + 6000).split(new RegExp("</" + tag + ">"))[0];
    // the COinS Z3988 blob is emitted as a SIBLING <span> right AFTER </cite>
    // (before </li>), so look a little past the close tag for it — but stop at
    // the next list item / CITEREF so we don't grab the following citation's.
    const tail = h
      .slice(m.index + entry.length, m.index + entry.length + 1000)
      .split(/<li[ >]|id="CITEREF/)[0];
    const cm = (entry + tail).match(/\btitle="(ctx_ver=Z39\.88[^"]*)"/);
    if (cm) citerefCoins[m[2]] = parseCoins(cm[1]);
    citerefArchive[m[2]] = citationArchiveInfo(entry);
    const t = stripTags(entry);
    if (t && t.length > 3 && !citerefText[m[2]]) citerefText[m[2]] = t;
    // in a cs1/cs2 citation the work title is the italicised run
    const iM = entry.match(/<i\b[^>]*>([\s\S]*?)<\/i>/);
    const iT = iM ? stripTags(iM[1]).trim() : "";
    if (iT && iT.length >= 4 && iT.length <= 240 && !citerefTitle[m[2]]) citerefTitle[m[2]] = iT;
  }
  ...
  function source(noteId) {
    ...
    // fall back to a bare source from a text string: prefer the full
    // bibliography entry a {{sfn}} points at ("Lintott, A. (1994a). "The Roman
    // empire and its problems…". In CAH² 9 (1994), pp. 16–39.") over the sfn
    // shorthand ("Lintott 1994a, pp. 25–26."); or a plain-text inline citation.
    const src = (n.citeref && citerefText[n.citeref]) || n.text;
    if (src && src.length > 3) {
      ...
      // author: "Lastname, First M. (year)." | "Lastname, First M." |
      // "Firstname M. Lastname." at the very start
      const authParenM = raw.match(/^([A-Z][A-Za-z'’-]+(?:,\s+[A-Z][A-Za-z.\s'’-]+?)?)\s*\(\s*\d{4}[a-z]?\s*\)/);
      const authM =
        authParenM ||
        raw.match(/^((?:[A-Z]\.?\s+|[A-Z][a-z]+\s+){0,3}[A-Z][a-z]+(?:,\s+(?:[A-Z]\.?\s*)+|,\s+[A-Z][a-z.\s]+)?)[.,]\s+/);
      const body = authM ? raw.slice(authM[0].length) : raw;
      // best title source, in order: the entry's italicised run, a quoted
      // title, then the first sentence of the body minus a leading
      // "(1994a)."-style date marker and any trailing page ref
      const quotedM = raw.match(/["“]([^"”]{8,220})["”]/);
      let title =
        (n.citeref && citerefTitle[n.citeref]) ||
        (quotedM ? quotedM[1].trim() : "") ||
        body
          .replace(/^[.,)]*\s*/, "")
          .replace(/^\(\s*\d{4}[a-z]?\s*\)\.?\s*/i, "")
          .split(/\.\s+/)[0]
          .replace(/\s*\bpp?\.\s*[\d–-]+\s*$/i, "")
          .trim();
      // reject a "title" that is really just a name + year that leaked through
      if (/^[A-Z][a-z]+(?:\s+[A-Z][a-z.]+){0,3}\s*\(?\s*\d{4}[a-z]?\s*\)?\.?$/.test(title)) title = "";
      if (!title || title.length < 3) title = raw.slice(0, 240);
      // "In <Container>" / "In: <Container>" after the title
      const contM = raw.match(/\bIn:?\s+([A-Z][^.,;]{3,80})/);
      return withArchive({
        author: authM ? authM[1].replace(/[.,]\s*$/, "").replace(/\s+/g, " ").trim() : null,
        title: title.slice(0, 240),
        container_work: contM ? contM[1].trim() : null,
        publisher_or_journal: contM ? contM[1].trim() : null,
...
function splitAtInlineCites(rawSentence) {
  ...
  // A split can strand a fragment that is not its own assertion — a trailing
  // adverbial phrase left behind because an *earlier* phrase in the same clause
  // carried a citation ("… at the Battle of Philippi[4] in October 42 BC,[5][6]"
  // splits after [4], leaving "in October 42 BC,[5][6]"). Anything shorter than
  // the claim-candidacy minimum can't stand alone, so glue it back onto its
  // neighbour instead of letting extractClaims drop it on the length check.
  const MIN_SEG = 25;
  const bare = (s) => s.replace(MARKER_STRIP, "").trim();
  const merged = [];
  for (const s of trimmed) {
    if (merged.length && bare(s).length < MIN_SEG) merged[merged.length - 1] += " " + s;
    else merged.push(s);
  }
  while (merged.length > 1 && bare(merged[0]).length < MIN_SEG) {
