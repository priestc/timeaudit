...
// pull inline citation markers out of a block of HTML, returning plain text with
// tokens label␟noteid where each marker was
...
function extractClaims(page, opts = {}) {
  ...
  while ((m = blockRe.exec(html))) {
    ...
    for (const rawSentence of splitSentences(plain)) {
      // separate markers from clean text
      const markers = [];
      const clean = rawSentence
        .replace(/([^␟]+)␟([^]+)/g, (_, label, noteId) => {
