...
const BARE_YEAR_RE =
  /\b(?:in|by|around|about|before|after|since|until|from|until|between|and|to|c\.|circa|dated|dating)\s+(?:the\s+)?(?:early\s+|mid-?\s*|late\s+)?(\d{3,4})s?\b(?!\s*(?:BC|BCE|AD|CE|ft|feet|foot|cm|mm|km|kg|ha|acres?|metres?|meters?|m\b|miles?|yards?|pounds?|manuscripts?|copies|copy|examples?|people|persons?|individuals?|graves?|burials?|artefacts?|artifacts?|items?|specimens?|samples?|sherds?|beads?|coins?|pieces?|fragments?|pillars?|stones?|structures?|sites?|houses?|km2|m2))/i;
...
function dateTriggers(text) {
  ...
  scan(BARE_YEAR_RE, 1); // just the year, not the leading "in"/"by"/…
  scan(PERIOD_RE_I);
  scan(PERIOD_RE_NAMED);
  return hits;
}
...
function assessCutoff(sentence) {
  ...
  // every parsed year is >= 1450, but a named period / bare century still points
  // at an ancient time
  if (PERIOD_RE_I.test(s) || PERIOD_RE_NAMED.test(s) || /\b\d{1,2}(?:st|nd|rd|th)\s+(?:century|millennium)\b/i.test(s)) {
