...
function extractClaims(page, opts = {}) {
  ...
  while ((m = blockRe.exec(html))) {
    ...
    for (const rawSentence of splitSentences(plain)) {
      ...
      if (!cut.inScope) {
        if (opts.includeRejected) rejected.push({ sentence: clean, section, reason: cut.basis });
        continue;
      }

      const key = clean.toLowerCase().replace(/[^a-z0-9]+/g, "");
      if (seen.has(key)) {
        if (opts.includeRejected) rejected.push({ sentence: clean, section, reason: "duplicate of an earlier in-scope sentence" });
        continue;
      }
      seen.add(key);
