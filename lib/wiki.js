...
const DATE_PATTERNS = [
  /\b(?:c\.|circa|around|about|approx\.?|roughly)?\s?\d{1,4}(?:,\d{3})?\s?(?:BC|BCE|B\.C\.(?:E\.)?)\b/i,
  /\b\d{3,4}\s?(?:AD|CE|A\.D\.|C\.E\.)\b/i,
  /\b\d{1,4}(?:,\d{3})?\s?[–-]\s?\d{1,4}(?:,\d{3})?\s?(?:BC|BCE|AD|CE)\b/i,
  /\b\d{1,2}(?:st|nd|rd|th)\s+(?:century|millennium)(?:\s+(?:BC|BCE|AD|CE))?\b/i,
  /\b(?:first|second|third|fourth|fifth|sixth|seventh|eighth|ninth|tenth|eleventh|twelfth|thirteenth|fourteenth)\s+(?:century|millennium)(?:\s+(?:BC|BCE|AD|CE))?\b/i,
...
const COPULA_PERIOD =
  /\b(?:is|was|are|were)\s+(?:an?\s+|the\s+)?(?:early\s+|middle\s+|late\s+|mature\s+)?(?:[A-Z][a-z]+\s+)?(?:epi[- ]?)?(?:pal(?:a)?eolithic|mesolithic|neolithic|chalcolithic|bronze[- ]age|iron[- ]age|stone[- ]age|prehistoric|medi(?:a)?eval)\b/i;
const PERIOD_NOUN =
  /\b(?:pal(?:a)?eolithic|mesolithic|neolithic|chalcolithic|bronze[- ]age|iron[- ]age|stone[- ]age|prehistoric)\s+(?:site|settlement|monument|structure|building|tomb|burial|complex|village|town|city|enclosure|temple|occupation|context|layer|deposit)/i;
...
const SITE_NOUN =
  /\b(sites?|monuments?|settlements?|complex(?:es)?|compounds?|structures?|buildings?|tells?|mounds?|enclosures?|circles?|henges?|sanctuar(?:y|ies)|temples?|pillars?|stones?|sarsens?|bluestones?|megaliths?|trilithons?|ruins?|remains?|tombs?|graves?|burials?|postholes?|ditch(?:es)?|banks?|avenues?|cursus|barrows?|cairns?|platforms?|phases?|stages?|layers?|strata|occupations?|constructions?|excavations?|deposits?|cities|city|towns?|cultures?)\b/i;
...
const ANAPHOR = /\b(it|its|they|their|these|this)\b/i;
...
function assessCutoff(sentence) {
  ...
  for (const m of s.matchAll(/\b(first|second|third|fourth|fifth|sixth|seventh|eighth|ninth|tenth|eleventh|twelfth|thirteenth|fourteenth)\s+(century|millennium)(?:\s+(BC|BCE|AD|CE))?\b/gi)) {
    ...
    if (/^bc/i.test(m[3] || "")) {
      years.push(-mid);
      sawBCE = true;
    } else if (/^(ad|ce)/i.test(m[3] || "")) years.push(mid);
    else ambiguous = true;
  }
  // N years ago / BP
  ...
  const ancientCue =
    /\b(BCE?|millennium|century)\b/i.test(s) || PERIOD_RE_I.test(s) || PERIOD_RE_NAMED.test(s);
  const bareModern = [...s.matchAll(/\b(1[5-9]\d\d|20\d\d)\b/g)].map((x) => +x[1]);

  if (!years.length) {
    if (bareModern.length && !ancientCue && !/\bcentury\b/i.test(s)) {
      return { inScope: false, basis: "only modern-era year(s) present (" + bareModern.join(", ") + ")", years, ambiguous: false };
    }
...
function extractClaims(page, opts = {}) {
  ...
  while ((m = blockRe.exec(html))) {
    ...
    if (/^(References|Notes|Citations|Bibliography|Sources|Further reading|External links|See also|Footnotes)$/i.test(section)) continue;
    ...
    for (let si = 0; si < sents.length; si++) {
      ...
      // Step 1: does the sentence mention a year / point in time at all?
      const hasNumericDate = DATE_PATTERNS.some((re) => re.test(clean));
      const hasPeriodWord = PERIOD_RE_I.test(clean) || PERIOD_RE_NAMED.test(clean);
      if (!hasNumericDate && !hasPeriodWord) continue; // not a candidate — not shown
      ...
      const record = { sentence: clean, sentence_cited: sentenceCited, section, markers, cutoff: cut };
      ...
      // Step 3: is the date/period used to *assign an age* to something?
      let ageUse = true;
      if (hasNumericDate) {
        // a real BC/AD date almost always dates something; reject only when it
        // is clearly placing a person or a modern event instead
        if (NON_DATING_CUE.test(clean) && !DATING_CUE.test(clean)) {
          ageUse = false;
        }
      } else {
        // period word only ("Neolithic") — needs an explicit dating cue
        ageUse = DATING_CUE.test(clean) || COPULA_PERIOD.test(clean) || PERIOD_NOUN.test(clean);
      }
      if (!ageUse) {
