...
// patterns that signal a numerical age claim
const DATE_PATTERNS = [
...
function assessCutoff(sentence) {
  ...
  const ancientCue = /\b(BCE?|millennium|Neolithic|Palaeolithic|Paleolithic|Mesolithic|Bronze Age|Iron Age|Chalcolithic|prehistor|antiquity|Pleistocene|Holocene|Stone Age)\b/i.test(s);
...
function extractClaims(page, opts = {}) {
  ...
  const drop = (sentence, section, reason) => {
    if (opts.includeRejected) rejected.push({ sentence, section, reason });
  };
  ...
  while ((m = blockRe.exec(html))) {
    ...
    for (const rawSentence of splitSentences(plain)) {
      const markers = [];
      ...
      if (!DATE_PATTERNS.some((re) => re.test(clean))) continue;

      const cut = assessCutoff(clean);
      if (!cut.inScope) {
        drop(clean, section, cut.basis);
        continue;
      }

      // a claim must be a single simple assertion, not a run of sentences
      if (clean.length > MAX_CLAIM_CHARS) {
        drop(clean, section, "sentence too long — not a single simple claim");
        continue;
      }
      ...
      if (!aboutSubject) {
        drop(clean, section, "not about the article subject");
        continue;
      }

      const key = clean.toLowerCase().replace(/[^a-z0-9]+/g, "");
      if (seen.has(key)) {
        drop(clean, section, "duplicate of an earlier in-scope sentence");
        continue;
      }
      seen.add(key);
