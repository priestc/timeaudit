...
const DATE_QUALIFIER_SRC =
  "(?:in|by|around|about|before|after|since|until|from|between|and|to|c\\.|circa|dated|dating|roughly|approx\\.?)";
// Step 1: patterns that count as "a mention of a year or point in time".
// numeric / calendar forms:
const DATE_PATTERNS = [
  new RegExp("\\b(?:" + DATE_QUALIFIER_SRC + "\\s+)?\\d{1,4}(?:,\\d{3})?\\s?(?:BC|BCE|B\\.C\\.(?:E\\.)?)\\b", "i"),
  new RegExp("\\b(?:" + DATE_QUALIFIER_SRC + "\\s+)?\\d{3,4}\\s?(?:AD|CE|A\\.D\\.|C\\.E\\.)\\b", "i"),
  /\b\d{1,4}(?:,\d{3})?\s?[–-]\s?\d{1,4}(?:,\d{3})?\s?(?:BC|BCE|AD|CE)\b/i,
  new RegExp(
    "\\b(?:" + DATE_QUALIFIER_SRC + "\\s+)?\\d{1,2}(?:st|nd|rd|th)\\s+(?:century|millennium)(?:\\s+(?:BC|BCE|AD|CE))?\\b",
    "i"
  ),
  new RegExp(
    "\\b(?:" +
      DATE_QUALIFIER_SRC +
      "\\s+)?(?:first|second|third|fourth|fifth|sixth|seventh|eighth|ninth|tenth|eleventh|twelfth|thirteenth|fourteenth)\\s+(?:century|millennium)(?:\\s+(?:BC|BCE|AD|CE))?\\b",
    "i"
  ),
...
const BARE_YEAR_RE = new RegExp(
  "\\b" +
    DATE_QUALIFIER_SRC +
    "\\s+(?:the\\s+)?(?:early\\s+|mid-?\\s*|late\\s+)?(\\d{3,4})s?\\b(?!\\s*(?:BC|BCE|AD|CE|ft|feet|foot|cm|mm|km|kg|ha|acres?|metres?|meters?|m\\b|miles?|yards?|pounds?|manuscripts?|copies|copy|examples?|people|persons?|individuals?|graves?|burials?|artefacts?|artifacts?|items?|specimens?|samples?|sherds?|beads?|coins?|pieces?|fragments?|pillars?|stones?|structures?|sites?|houses?|km2|m2))",
  "i"
);
