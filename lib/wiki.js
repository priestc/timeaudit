...
function assessCutoff(sentence) {
  ...
  // at an ancient time — unless the period word only appears inside a quoted
  // phrase ("postulating an origin in 'remote antiquity'"), which is someone's
  // characterization, not a dating claim, and shouldn't rescue a modern sentence
  const sUnquoted = s.replace(/"[^"]*"|“[^”]*”/g, " ");
  if (
    PERIOD_RE_I.test(sUnquoted) ||
    PERIOD_RE_NAMED.test(sUnquoted) ||
    /\b\d{1,2}(?:st|nd|rd|th)\s+(?:century|millennium)\b/i.test(sUnquoted)
  ) {
