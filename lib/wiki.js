...
async function fetchPage(input, opts = {}) {
  ...
  if (opts.cacheDir) {
    ...
    // rendered PDF of the article (Wikimedia "Download as PDF" / Proton) — used
    // for the per-claim screenshot of where the sentence sits on the page
    const pdfFile = path.join(dir, slug + ".pdf");
    try {
      if (!fs.existsSync(pdfFile)) {
        const buf = await getBuffer(
          "https://" + lang + ".wikipedia.org/api/rest_v1/page/pdf/" +
            encodeURIComponent(p.title.replace(/ /g, "_"))
        );
        if (buf && buf.slice(0, 5).toString("latin1") === "%PDF-") fs.writeFileSync(pdfFile, buf);
      }
      if (fs.existsSync(pdfFile)) {
        out.pdfPath = pdfFile;
        out.cachePaths.pdf = path.join("source-cache", "_wikipedia", slug + ".pdf");
      }
    } catch (e) {
      process.stderr.write("  (could not fetch the Wikipedia PDF render: " + e.message + ")\n");
    }
  }
  return out;
}
...
const ABBREV = /\b(?:c|ca|approx|no|vol|pp|ed|eds|Dr|Mr|Mrs|St|Fig|e\.g|i\.e|cf|r|fl|b|d)\.$/i;
...
function splitSentences(text) {
  ...
  for (let i = 0; i < toks.length; i++) {
    buf += toks[i];
    if (/[.!?]["')\]]?$/.test(toks[i]) && !ABBREV.test(buf) && !/\b[A-Z]\.$/.test(buf.trim())) {
      parts.push(buf.trim());
      buf = "";
    }
  }
...
function extractClaims(page, opts = {}) {
  ...
  while ((m = blockRe.exec(html))) {
    ...
    if (/^(References|Notes|Citations|Bibliography|Sources|Further reading|External links|See also)$/i.test(section)) continue;
    ...
    for (const rawSentence of splitSentences(plain)) {
      ...
      if (opts.maxClaims && claims.length >= opts.maxClaims) return claims;
    }
  }
  return claims;
}
