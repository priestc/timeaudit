...
| **Quote screenshots** | For each final verbatim quote from a cached PDF, `pdftotext -bbox-layout` locates the passage and `pdftoppm` crops a PNG of exactly that region — the quote in the source's own typography. Saved as `source-cache/<slug>/<src>.h<hop>q<n>.png`; path recorded in `quote_images[]`, aligned to `verbatim_quotes[]` (null where the passage couldn't be located or the source isn't a PDF). Shown under "Verbatim quotes" in the viewer; inlined as `data:` URIs in standalone HTML. |
