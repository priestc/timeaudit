...
| 1 | **Citation metadata** | Every Wikipedia citation embeds a COinS / OpenURL blob (`class="Z3988"`) — author, title, year, journal, DOI, ISBN, pages, all machine-readable. Shortened `{{sfn}}` footnotes are followed to their bibliography entry (`<cite>` *and* the bare `<span id="CITEREF…">` form, using the italicised run as the work title). Lettered `{{efn}}` explanatory notes (`[m]`/`[n]`) are not citations — their quoted snippet is attached to the numbered source they quote as `hop.wikipedia_note_quotes`. No scraping guesswork. |
| 1→2 | **Metadata enrichment** | When a citation still has no URL and no DOI (a bare `{{sfn}}` to a book chapter, a print-only reference), the work is looked up — the scriptable form of "search for it and read the results" — in **Google Books** first (best book coverage; recovers title / author / year / publisher / ISBN + a volume id), then **Crossref** (`query.bibliographic`), then **Open Library** if Crossref drew a blank. A confident match (≥3 distinctive title words overlapping + author surname + year; for a book citation, book-shaped Crossref types only so a *review* of the book can't masquerade as it) fills the citation's missing fields; a recovered DOI / ISBN / Google Books id then feeds retrieval (the ISBN'd Internet Archive search, the DOI landing page, the Google Books PDF). Every lookup — hit or miss — is logged to `source.metadata_history`. The keyless Google Books quota is a small shared daily pool; set `GOOGLE_BOOKS_API_KEY` in `.env` to make that step reliable. |
...
| 2 | **Download sources** | For each cited source, candidate URLs are tried in order: direct PDF → **Wikipedia `archive-url`** (the editor-recorded snapshot) → **OpenAlex** OA location (by DOI, no key) → **Europe PMC** full-text XML (by DOI/PMCID, no key) → **NCBI BioC** XML (`…/pmcoa.cgi/BioC_xml/<PMCID>/unicode`, for any PMC-OA / author-manuscript article — the PMC website and `oa.fcgi` are behind NCBI's WAF and 403 every datacenter IP, but BioC is not) → **Unpaywall** (needs `--email`) → **Internet Archive** (for book-shaped citations — by ISBN, else title + author surname over `mediatype:texts`; the item's full-text PDF then its OCR `.txt`; this is how a public-domain book cited only via a Google Books preview link actually gets fetched) → the cited URL → `doi.org`. Responses are gunzipped/brotli-decoded before caching (the Wayback raw endpoint replays original compressed bytes without a `Content-Encoding` header). Polite: 1 request/second/host, one User-Agent, 40 MB cap, already-cached files reused (SPEC rule 8). Bot-walls / paywalls / Cloudflare challenges / login redirects are detected and skipped. When every candidate fails, the *most specific* reason found across all of them is kept — a `books.google.*` link (never anything but a restricted preview for an in-copyright book) records `"unable to retrieve source because copyrighted"`; other paywalls, bot-challenges, HTTP errors, login walls, and print-only citations with no online copy at all each get their own accurate `source.retrieval_note`, not a blanket "unreachable". |
