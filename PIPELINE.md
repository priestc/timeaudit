...
| 1→2 | **Metadata enrichment** | When a citation still has no URL and no DOI (a bare `{{sfn}}` to a book chapter, a print-only reference), the work is looked up in **Crossref** (`query.bibliographic`) and, for books, **Open Library** — the scriptable form of "search for it and read the results". A confident match (title-word overlap + author surname + year, book-shaped Crossref types only so a *review* of the book can't masquerade as it) fills the citation's missing title / author / year / container / DOI / ISBN, and the recovered DOI then feeds retrieval. Every lookup — hit or miss — is logged to `source.metadata_history`. |
